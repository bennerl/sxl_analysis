library("ballgown")
mCh <- list.files("~/sxl_data/stringtie_merge/", pattern = "mCh.*", full.names = T)
Sxl <- list.files("~/sxl_data/stringtie_merge/", pattern = "Sxl.*", full.names = T)
BAMF <- list.files("~/sxl_data/stringtie_merge/", pattern = "bamF.*", full.names = T)
BAMM <- list.files("~/sxl_data/stringtie_merge/", pattern = "bamM.*", full.names = T)
...
eTOg <- eTOg[,-c(1)]
eTOg <- unique(eTOg)
write.table(tTOg, "~/sxl_data/data_tables/t2g_unannotated.txt", quote = F, sep = "\t", row.names = F, col.names = T)
write.table(eTOg, "~/sxl_data/data_tables/e2g_unnannotated.txt", quote = F, sep = "\t", row.names = F, col.names = T)
...
write.table(gene_exp_table, "~/sxl_data/data_tables/raw_gene_table_unannotated.txt", row.names = F, col.names = T, sep = "\t", quote = F)
write.table(trans_exp_table, "~/sxl_data/data_tables/raw_trans_table_unannotated.txt", row.names = F, col.names = T, sep = "\t", quote = F)
write.table(exon_exp_table, "~/sxl_data/data_tables/raw_exon_table_unannotated.txt", row.names = F, col.names = T, sep = "\t", quote = F)
