exon <- read.delim("~/bam_sxl_analysis/data_tables/raw_exon_table_unannotated.txt", header = T)
...
write.table(exon_pos_fc, "~/bam_sxl_analysis/data_tables/exon_pos_fc_unannot.txt", row.names = F, col.names = T, quote = F, sep = "\t")
write.table(exon_neg_fc, "~/bam_sxl_analysis/data_tables/exon_neg_fc_unannot.txt", row.names = F, col.names = T, quote = F, sep = "\t")
